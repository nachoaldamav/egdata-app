    ...
    <div key={hit.id}className='rounded-md pl-2 pr-2 game_card'>
                    <Link href={`/product/${hit.slug}`} target="">
                        ...
                        <div className="px-2 py-2">
                            <div className="font-bold text-gray-50 text-base mb-2 game_title">{hit.title}</div>
                            <div className="font-bold text-gray-400 text-sm mb-2 game_title">{hit.seller}</div>
                            <div className="text-gray-200 text-base">
                            {hit.discount > 0 &&
                                <div className="text-gray-500 text-base inline font-sans">
                                    <span className="tracking-wider leading-relaxed text-xs p-1 bg-blue-500 font-medium rounded-md no-underline text-white">-{hit.discountPercentage}%</span>
                                    <div className="inline"> </div>
                                    <div className="line-through inline">{hit.originalPrice}</div>
                                </div>
                            }
                                <div className="text-gray-100 text-base inline"> {hit.currentPrice}</div>
                            </div>
                        </div>
                        </a>
                    </Link>
